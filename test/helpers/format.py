...
def _safe_int(signal: str, base=8):
...
def _format_register(register: int) -> str:
    if register < 13:
        return f"R{register}"
    if register == 13:
        return f"%blockIdx"
...
def _format_memory_controller_state(state) -> str:

    a = _safe_int(str(state), 2)
    if a is None:
        return "X"
...
def _format_cache(memory_controller):
    ...
    CACHE_LINE_SIZE_BITS = int(
        str(memory_controller.CACHE_LINE_SIZE_BITS.value), 10)
    CACHE_TAG_BITS = int(str(memory_controller.CACHE_TAG_BITS.value), 10)
    NUM_CHUNKS = int(str(memory_controller.NUM_CHUNKS.value), 10)
...
def _safe_getattr(obj, attr_chain, default="N/A"):
    try:
...
def _log_if_changed(tag, val, header_fn=None):
...
def _walk_dut_limited(handle, prefix="", depth=0, max_depth=4, header_state={"printed": False}):
    if depth > max_depth:
        return
    ...
    for name in dir(handle):
        ...
        try:
            ...
            if hasattr(sub, "value"):
                ...
                _walk_dut_limited(sub, full_name, depth + 1,
...
def format_cycle(dut, cycle_id: int):
    logger.debug(f"\n========== Cycle {cycle_id} ==========")
    _walk_dut_limited(dut)

    gpu_vals = {
        "dut done": str(_safe_getattr(dut, "done")),
        "dut start": str(_safe_getattr(dut, "start")),
        "logical core start sigs": str(_safe_getattr(dut, "core_start")),

        "dispatcher done": str(_safe_getattr(dut, "dispatch_instance.done")),
        "total_blocks": _safe_int(str(_safe_getattr(dut, "dispatch_instance.total_blocks"))),
        "blocks_dispatched": _safe_int(str(_safe_getattr(dut, "dispatch_instance.blocks_dispatched"))),
        "blocks_done": _safe_int(str(_safe_getattr(dut, "dispatch_instance.blocks_done"))),
        "start_execution": str(_safe_getattr(dut, "dispatch_instance.start_execution")),

        # Dmem controller and consumers
        **{
            f"Dmem controller_state({i})": _format_memory_controller_state(
                _safe_getattr(
                    dut, f"data_memory_controller.controller_state[{i}]")
            ) for i in range(4)
        },
        **{
            f"Dmem consumer_state({i})": _format_memory_controller_state(
                _safe_getattr(
                    dut, f"data_memory_controller.consumer_state[{i}]")
            ) for i in range(8)
        },
        "Dmem mutex": str(_safe_getattr(dut, "data_memory_controller.consumer_mutex")),
        "Dmem active main_mem requests": str(_safe_getattr(dut, "data_memory_controller.main_mem_request")),
        "Dmem cache": _format_cache(_safe_getattr(dut, "data_memory_controller")),
        "Dmem list": str(_safe_getattr(dut, "data_memory_controller")),

        # Pmem controller
        "pmem controller_state": _format_memory_controller_state(
            _safe_getattr(
                dut, "program_memory_controller.controller_state[0]")
        ),
        **{
            f"pmem consumer_state({i})": _format_memory_controller_state(
                _safe_getattr(
                    dut, f"program_memory_controller.consumer_state[{i}]")
            ) for i in range(2)
        },
        "pmem cache": _format_cache(_safe_getattr(dut, "program_memory_controller")),
        "pmem mutex": str(_safe_getattr(dut, "program_memory_controller.consumer_mutex")),
        "pmem cons0 line valid": str(_safe_getattr(dut, "program_memory_controller.consumer_if[0].req_line_valid")),
        "pmem cons0 tag valid": str(_safe_getattr(dut, "program_memory_controller.consumer_if[0].tag_valid")),
        "pmem cons1 line valid": str(_safe_getattr(dut, "program_memory_controller.consumer_if[1].req_line_valid")),
        "pmem cons1 tag valid": str(_safe_getattr(dut, "program_memory_controller.consumer_if[1].tag_valid")),
        "pmem list": str(_safe_getattr(dut, "program_memory_controller")),
    }
    ...
    for key, val in gpu_vals.items():
        _log_if_changed((dut, key), val, print_gpu_header)
    ...
    # total_threads = _safe_int(dut.thread_count.value, 2)
    # threads_per_block = _safe_int(dut.THREADS_PER_BLOCK.value, 10)
    ...
    #            "Instruction": _format_instruction(str(logical_core.instruction.value)),
    #            "PC": safe_hex(logical_core.current_pc.value),
    #            "Core State": _format_core_state(str(logical_core.core_state.value)),
    #            "Fetcher State": format_fetcher_state(logical_core),
    #            "Decoded Immediate": _safe_int(logical_core.decoded_immediate.value, 2),
    #        }

    #        for key, val in logical_core_values.items():
    #            _log_if_changed((logical_core, key), val, print_lc_header)

    #        # Handle each thread
    #        for thread in logical_core.threads:
    #            tv = _safe_int(thread.i.value, 10)
    #            tc = _safe_int(logical_core.thread_count.value, 2)
    #            if None in (tv, tc):
    #                continue
    #            elif tv >= tc:
    #                continue
    ...
    #                _log_if_changed((idx, key), val, print_thread_header)

    #            for i, reg in enumerate(thread.register_instance.registers):
    #                tag = (idx, f"reg_{i}")
    #                val = str(reg.value)
    #                _log_if_changed(tag, val, print_thread_header)
    #


def _snapshot_cache(memory_controller):
    try:
        raw_lines = _format_cache(memory_controller)
        if raw_lines == "X":
            return None
...
def _format_instruction(instruction: str) -> str:
    ...
    rd = _format_register(int(instruction[4:8], 2))
    rs = _format_register(int(instruction[8:12], 2))
    rt = _format_register(int(instruction[12:16], 2))
...
def _format_core_state(core_state: str) -> str:
    core_state_map = {
        "000": "IDLE",
        "001": "FETCH",
        "010": "DECODE",
...
def _format_fetcher_state(logical_core) -> str:
...
def _format_registers(registers: List[str]) -> str:
    ...
    for i, reg_value in enumerate(registers):
        ...
        formatted_registers.append(
            f"{_format_register(reg_idx)} = {decimal_value}")
